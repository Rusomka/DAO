...
public abstract class AbstractDAO<T> {
    ...
    public void add(T t) {
        try {
            ...
            try (Statement st = conn.createStatement()) {
                st.execute(sql);
                //add in field 'id' - primary key from databases{
                /*
                    CREATE DEFINER=`root`@`localhost` PROCEDURE `test2`(OUT res INT)
                    BEGIN
                    SET res= last_insert_id();
                    END
                 */
                try (CallableStatement cs = conn.prepareCall("CALL test2(?)")) {
                    cs.setString(1, "@t");
                    cs.registerOutParameter(1, Types.INTEGER);
                    cs.execute();
                    id.set(t, cs.getInt(1));
                }
                //...}
            }
    ...
    public List<T> getAll(Class<T> cls, String... str) {
        ...
        if (str.length != 0) {
            for (String t : str) {
                if (t.equals("name") || t.equals("age") || t.equals("id")) {
                    sql.append(t).append(",");
                } else {
                    sql.append("* ");
                    temp.append(getParss(str));  // priavte method...
                }
            }
            sql.deleteCharAt(sql.length() - 1);
        } else
        ...
        try {
            try (Statement st = conn.createStatement()) {
                try (ResultSet rs = st.executeQuery("SELECT " + sql + " FROM " + table + temp)) {
    ...
    private StringBuilder getParss(String... str) {
        StringBuilder temp = new StringBuilder();
        temp.append(" where");
        for (String t : str) {
            if (t.trim().matches("\\d+")) {
                temp.append(" age=").append("\'").append(t).append("\';");
            } else
                temp.append(" name=").append("\'").append(t).append("\';");
        }
        return temp;
    }
